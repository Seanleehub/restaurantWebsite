...
<script language="javascript" type="text/javascript" src="javascript/valdel.js"></script>
...
<div id="leftleft">
    ...
    <div id="rightcontent">
      <h2><!-- InstanceBeginEditable name="header" -->Enter the Date of the reservations you want to delete.<!-- InstanceEndEditable --></h2>
      <!-- InstanceBeginEditable name="content" -->
      <div id="content">
        <h1>List of reservations on        </h1>
        <form action="delete.php" method="get" onsubmit="return ValidateForm(this);">
                      <td width="22%">  <input type="text" size="20" name="date" value="<?php if(ISSET($_SESSION['date'])){print $_SESSION['date'];}?>"/> </td>
...
foreach($_GET as $id) { // This will loop through the checked checkboxes

mysql_query("DELETE FROM reservation WHERE id='$id'"); // Change yourtable and id. This deletes the record from the database
...
<br />
  ...
  <form name="action" id="action" method="get" action="delete.php"> 
  	<input type="hidden" name="date" value="<?php if(ISSET($_SESSION['date'])){ print $_SESSION['date'];}?>" />
<?php
...
<td>Name</td>
		...
		while($row = mysql_fetch_array($result)){
				...
				print "<td>$row[5]</td>";
				print "<td>$row[6]</td>";
				print "<td><input type=\"checkbox\" name=\"$row[id]\" id=\"$row[id]\" value=\"$row[id]\" /></td>";
...
if($_POST['perform']) {
print "Selected Items were deleted";
